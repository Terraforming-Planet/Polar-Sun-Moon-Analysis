(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .simple-agentic-actions button,.simple-agentic-actions a{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:9px 14px;border-radius:8px;border:1px solid rgba(56,200,255,.45);background:rgba(10,62,89,.62);color:#eaf9ff;text-decoration:none;font:inherit;font-size:.86rem;cursor:pointer;touch-action:manipulation}
  ...
  function ensureOverview() {
    ...
    section.innerHTML = `
      ...
      <div class="simple-agentic-evidence">
        ...
        <article><h3>How the agents work together</h3><p>The Coordinator receives the research objective, asks Source Scout which controlled EO sources can answer it, asks Evidence Verifier what the repository evidence actually supports, and only then prepares a response with uncertainty and next checks. This makes the workflow inspectable rather than a single opaque model answer.</p></article>
        <article><h3>What TEST 014 does not prove</h3><p>The current evidence does not by itself prove long-term water loss, blocked river flow or a causal mechanism. Those questions require reproducible image measurements, matched seasons, hydrological observations and checks against alternative explanations.</p></article>
      </div>
    ...
    const openButton = section.querySelector('[data-open-agentic-eo]')
    openButton?.addEventListener('click', openAgenticEO)
