...
def write_pair(root: Path, name: str, *, generated: bool = False, size: int = 8) -> None:
    before = np.zeros((4, size, size), dtype=np.float32)
...
def test_cached_pairs_require_real_provenance_and_preserve_unique_count(tmp_path: Path) -> None:
    ...
    write_pair(tmp_path, "pair-b", size=16)
    arrays, targets, provenance = load_cached_pairs(tmp_path, max_pairs=10)
    assert [array.shape for array in arrays] == [(8, 8, 8), (8, 16, 16)]
