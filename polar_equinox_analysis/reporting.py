    ...
    for quantity, ylabel in figure_specs.items():
        ...
        for axis, pole in zip(axes, sorted(observations["pole"].unique()), strict=True):
            ...
            for (season, body), group in subset.groupby(["season", "body"]):
                ...
                if len(group) > 2:
                    coeffs = np.polyfit(group["year"], group[quantity], 1)
                    axis.plot(
                        group["year"],
                        coeffs[0] * group["year"] + coeffs[1],
                        linestyle="--",
                        alpha=0.5,
                    )
            axis.set_title(pole)
...
def _create_heatmap(observations: pd.DataFrame, figures_dir: Path, paths: list[Path]) -> None:
    ...
    pivot = observations.pivot_table(
        index="year", columns=["pole", "season", "body"], values="apparent_altitude_deg"
    )
...
def export_markdown_report(
    observations: pd.DataFrame, statistics: pd.DataFrame, summary: str, out_dir: Path
) -> Path:
...
## Introduction
This report observes the seasonal apparent altitude of the Sun and Moon above the local horizon at Earth's
poles during equinoxes.
...
## Methodology
NASA API → Validation → Cache → Raw Data Archive → Cleaning → Quality Control → Statistical Analysis →
Figures → Report → Website.
...
## Discussion
The software performs observations only. It does not prove hypotheses; interpretations must reference
computed Horizons observations.

## Limitations
Results are limited to requested dates, locations, API quantities, and Horizons availability.

## Future Work
Additional observation periods, observatories, bodies, and statistical methods can be added through
...
def build_website(out_dir: Path, website_dir: Path) -> None:
    ...
    sections = "".join(
        f'<section id="{p.lower().replace(" ", "-")}"><h2>{p}</h2><p>This section focuses on validated'
        f" NASA JPL Horizons observations of seasonal apparent Sun and Moon altitude during equinoxes."
        f" Established facts are separated from research hypotheses; the software observes only and does"
        f" not prove hypotheses.</p></section>"
        for p in pages
    )
    html = f"""<!doctype html>
<html>
...
<title>Polar Sun Moon Analysis</title>
<style>
body{{margin:0;font-family:system-ui;background:#0b1020;color:#e8eefc}}
header{{padding:3rem;background:linear-gradient(135deg,#0b1020,#17345f)}}
nav{{display:flex;flex-wrap:wrap;gap:.75rem;padding:1rem;background:#11182b;position:sticky;top:0}}
a{{color:#9bdcff}}
section{{max-width:1000px;margin:auto;padding:2rem}}
.card{{background:#151f36;border:1px solid #2c3e66;border-radius:16px;padding:1rem}}
</style>
</head>
<body>
<header>
<h1>Polar Equinox Sun/Moon Horizons Analysis</h1>
<p>Production scientific workflow for seasonal apparent altitude observations during Earth's equinoxes.</p>
</header>
<nav>{nav}</nav>
<main>
{sections}
...
<h2>Downloads</h2>
<ul>
<li>observations.csv</li>
<li>statistics.csv</li>
<li>polar_equinox_report.pdf</li>
</ul>
</section>
</main>
</body>
    ...
    (website_dir / "pipeline.md").write_text(
        "# Pipeline\n\nNASA API → Validation → Cache → Raw Data Archive → Cleaning → Quality Control →"
        " Statistical Analysis → Figures → Report → Website\n",
        encoding="utf-8",
    )
