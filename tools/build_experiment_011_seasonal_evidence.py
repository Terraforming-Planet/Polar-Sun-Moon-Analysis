...
# The 65 x 60 km regional frame can cross Landsat WRS footprints and
# Sentinel-2 MGRS tiles.  Building it from one source item creates genuine
# no-data borders.  These caches assemble each output only from real source
# pixels from neighboring acquisitions.  No generative fill is permitted.
...
def _candidate_mosaic_items(primary: dict) -> list[dict]:
    ...
    try:
        features = m.base.request_json("POST", m.base.PC_SEARCH, json=payload).get(
            "features", []
        )
    ...
    def rank(item: dict) -> tuple:
        item_id = str(item.get("id", ""))
        try:
            distance = abs((date.fromisoformat(m.base.item_date(item)) - anchor).days)
        except Exception:
            distance = 999
...
def _unsigned_href(href: str) -> str:
    # Never keep an old Planetary Computer SAS signature in a cached href.
...
def _read_one_item(
    ...
    for attempt in range(4):
        if attempt:
            # Long 74-slot builds can outlive a SAS token.  Force a new token
            # before retrying rather than accepting a missing/black raster.
            ...
            with rasterio.Env(**env):
                with rasterio.open(href) as src:
                    reproject(
                        ...
                        dst_nodata=np.nan,
                        resampling=(
                            Resampling.nearest if nearest else Resampling.bilinear
                        ),
                    )
            return dst
        except Exception as exc:
...
def mosaic_read_asset(
    ...
    del resolution_m  # Test 011 intentionally uses one uniform 30 m grid.
    ...
    if not np.any(np.isfinite(merged)):
        raise RuntimeError(
            f"regional mosaic contains no readable pixels for {primary_id} {key}; "
...
def _set_full_aoi_search_bbox() -> None:
    ...
    print("TEST011 full AOI search bbox", m.base.SEARCH_BBOX, flush=True)
...
def normalize_season_output(name: str, result: dict) -> dict:
    root = SEASONS / name
    old = root / (
        f"EXPERIMENT_001_{name.upper()}_1990_2026_37_YEARS_2km_53.591400_19.010717.zip"
    )
    if old.exists():
        old.unlink()
    new = root / (
        f"TEST011_{name.upper()}_1990_2026_37_YEARS_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    )
    ...
    manifest.update(
        {
            ...
            "zip": str(new),
            "source_policy": "real public satellite pixels only; USGS Landsat Collection 2 and ESA/Copernicus Sentinel-2 products; multi-scene regional mosaics use only real acquisitions; no generative fill or AI super-resolution",
            "regional_mosaic_policy": {
                "enabled": True,
                "reason": "65x60 km AOI can cross Landsat WRS and Sentinel-2 MGRS boundaries",
...
- Fixed frame: **65 km × 60 km**, north-up, UTM 34N.
...
This package is evidence acquisition. Quantitative shoreline/area/volume conclusions require the later spectral-water measurement stage and hydrological data. No generative filling or AI super-resolution is used.
"""
...
def build_combined_zip() -> Path:
    out = EXP / (
        f"TEST011_ILAWA_ZALEWO_1990_2026_SPRING_AUTUMN_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    )
    if out.exists():
        out.unlink()
...
def main() -> None:
    ...
    normalize_season_output("spring", m.exp1.build_season("spring", [5, 4, 6]))
    normalize_season_output("autumn", m.exp1.build_season("autumn", [9, 10, 11]))
    write_metadata()
