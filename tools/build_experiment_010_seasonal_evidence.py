...
BRANCH = "experiment-010-drweca-wetlands-53-067318-18-872006"
EXP = Path("experiments/experiment_010_drweca_wetlands")
SEASONS = EXP / "seasonal_evidence"
TARGET = {"name": "Dolina Drwecy", "lat": LAT, "lon": LON}
FRAME_WIDTH_M = 8000.0
FRAME_HEIGHT_M = 12000.0
FRAME_LABEL = "8x12km"
...
def normalize_season_output(name: str, result: dict) -> dict:
    ...
    if old.exists():
        old.unlink()
    new = root / f"TEST010_{name.upper()}_1990_2026_37_YEARS_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    manifest_path = root / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "experiment": "Experiment 010 - Drweca floodplain and wetland monitoring",
            ...
            "aoi": {
                ...
                "purpose": "wide reference-height view covering the meandering Drweca corridor, floodplain, oxbow/wetland areas, forest and agricultural context visible around the requested point",
            },
            "dataset_role": "independent TerraWater river-wetland comparison case",
            "test_number": 10,
            "object_name": "Dolina Drwecy",
            "zip": str(new),
        }
    )
...
def write_metadata(spring: dict, autumn: dict) -> None:
    ...
    sy = m.accepted_years(sm)
    ay = m.accepted_years(am)
    sp = [y for y in YEARS if y not in sy]
    ap = [y for y in YEARS if y not in ay]
    platforms = m.platform_list(sm, am)
    policy = {
        "experiment_id": "010",
        "object": "Dolina Drwecy",
        "center": {"lat": LAT, "lon": LON},
        "target": TARGET,
        "frame": {"width_m": 8000, "height_m": 12000, "orientation": "portrait_north_up"},
        ...
        "rules": {
            ...
            "river_corridor_and_floodplain_must_be_inside_frame": True,
        ...
        "measurement_status": "not_started; evidence acquisition only",
    }
        ...
        json.dumps(
            {
                ...
                "frame": {"width_m": 8000, "height_m": 12000, "orientation": "portrait_north_up"},
                "years": YEARS,
                "spring_missing_years": sp,
                "autumn_missing_years": ap,
    ...
    report = f"""# Experiment 010 — Dolina Drwęcy satellite evidence 1990–2026

- Object: **Dolina Drwęcy / meandering river and wetlands**
- Center: **{LAT_STR}, {LON_STR}**
- Frame: **8 km × 12 km**, portrait, north-up
- Wide reference view preserving the meandering river corridor, floodplain, wet meadows / oxbows and surrounding forest-agricultural context.
...
Evidence acquisition only; no wetland-loss or hydrological-cause conclusion before QA and measurement.
"""
    (EXP / "EXPERIMENT_010_REPORT.md").write_text(report, encoding="utf-8")


def build_combined_zip() -> Path:
    out = EXP / f"TEST010_DRWECA_WETLANDS_1990_2026_FULL_VIEW_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    if out.exists():
        out.unlink()
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        for extra in ("EXPERIMENT_010_REPORT.md", "experiment.json", "EVIDENCE_POLICY.json"):
            p = EXP / extra
            if p.exists():
                zf.write(p, p.name)
    return out
...
def main() -> None:
    ...
    summary = {
        ...
        "platforms_used": m.platform_list(sm, am),
