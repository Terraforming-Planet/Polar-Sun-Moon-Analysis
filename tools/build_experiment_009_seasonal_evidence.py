...
BRANCH = "experiment-009-jezioro-plowez-53-452516-19-212111"
EXP = Path("experiments/experiment_009_jezioro_plowez")
SEASONS = EXP / "seasonal_evidence"
TARGET = {"name": "Jezioro Plowez", "lat": LAT, "lon": LON}
...
def normalize_season_output(name: str, result: dict) -> dict:
    ...
    if old.exists():
        old.unlink()
    new = root / f"TEST009_{name.upper()}_1990_2026_37_YEARS_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    manifest_path = root / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "experiment": "Experiment 009 - Jezioro Plowez water and shoreline monitoring",
            ...
            "aoi": {
                ...
                "purpose": "reference-height view covering all of Jezioro Plowez plus nearby Plowez, Rywaldzik, wetlands, small ponds and surrounding agricultural/forest context",
            },
            "dataset_role": "independent TerraWater lake comparison case",
            "test_number": 9,
            "object_name": "Jezioro Plowez",
            "zip": str(new),
        }
    )
...
def write_metadata(spring: dict, autumn: dict) -> None:
    ...
    report = f"""# Experiment 009 — Jezioro Płowęż satellite evidence 1990–2026

- Object: **Jezioro Płowęż**
- Center: **{LAT_STR}, {LON_STR}**
- Frame: **7 km × 9 km**, portrait, north-up
- Wide reference view preserving the whole lake and surrounding Płowęż / Rywałdzik / wetlands / ponds / fields.
...
"""
    (EXP / "EXPERIMENT_009_REPORT.md").write_text(report, encoding="utf-8")


def build_combined_zip() -> Path:
    out = EXP / f"TEST009_JEZIORO_PLOWEZ_1990_2026_FULL_VIEW_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    if out.exists():
        out.unlink()
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        for extra in ("EXPERIMENT_009_REPORT.md", "experiment.json", "EVIDENCE_POLICY.json"):
            p = EXP / extra
            if p.exists():
                zf.write(p, p.name)
    return out
