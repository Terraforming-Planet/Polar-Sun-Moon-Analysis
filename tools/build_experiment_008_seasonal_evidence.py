...
BRANCH = "experiment-008-jezioro-stelchno-53-525217-18-461320"
EXP = Path("experiments/experiment_008_jezioro_stelchno")
SEASONS = EXP / "seasonal_evidence"
TARGET = {"name": "Jezioro Stelchno", "lat": LAT, "lon": LON}
...
def configure_globals() -> None:
    ...
    m.DISPLAY_WIDTH = 1000; m.DISPLAY_HEIGHT = 1286
...
def normalize_season_output(name: str, result: dict) -> dict:
    ...
    new = root / f"TEST008_{name.upper()}_1990_2026_37_YEARS_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    manifest_path = root / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest.update({
        "experiment": "Experiment 008 - Jezioro Stelchno water and shoreline monitoring",
        "center": {"lat": LAT, "lon": LON}, "targets": [TARGET],
        "aoi": {"width_m": 7000, "height_m": 9000, "orientation": "portrait_north_up", "purpose": "reference-height view covering all of Jezioro Stelchno plus Skrzynki, Czersk Swiecki, Lipinki and surrounding wetland/agricultural context"},
        "dataset_role": "independent TerraWater lake comparison case", "test_number": 8,
        "object_name": "Jezioro Stelchno", "zip": str(new),
    })
...
def write_metadata(spring: dict, autumn: dict) -> None:
    ...
    policy = {"experiment_id":"008","object":"Jezioro Stelchno","center":{"lat":LAT,"lon":LON},"target":TARGET,"frame":{"width_m":7000,"height_m":9000,"orientation":"portrait_north_up"},"years":{"start":1990,"end":2026,"count":37},"season_policy":{"spring":{"preferred_month":5,"fallback_months":[4,6]},"autumn":{"preferred_month":9,"fallback_months":[10,11]}},"rules":{"real_acquisition_date_required":True,"fallback_must_be_labeled":True,"future_observations_never_invented":True,"autumn_2026_must_remain_missing_until_observed":True,"same_frame_for_every_year":True,"whole_lake_and_context_must_be_inside_frame":True,"google_maps_reference_not_measurement_evidence":True,"generative_fill_forbidden":True,"ai_super_resolution_forbidden":True,"cross_year_exact_duplicate_rejected":True},"primary_sources":["USGS Landsat Collection 2","ESA/Copernicus Sentinel-2 Level-2A"],"radar_control_planned":"ESA/Copernicus Sentinel-1 RTC","measurement_status":"not_started; evidence acquisition only"}
    (EXP / "EVIDENCE_POLICY.json").write_text(json.dumps(policy, indent=2, ensure_ascii=False), encoding="utf-8")
    (EXP / "experiment.json").write_text(json.dumps({"experiment_id":"008","name":"Jezioro Stelchno","center":{"lat":LAT,"lon":LON},"target":TARGET,"frame":{"width_m":7000,"height_m":9000,"orientation":"portrait_north_up"},"years":YEARS,"spring_missing_years":sp,"autumn_missing_years":ap,"platforms_used":platforms}, indent=2, ensure_ascii=False), encoding="utf-8")
    report=f"""# Experiment 008 — Jezioro Stelchno satellite evidence 1990–2026

- Object: **Jezioro Stelchno**
- Center: **{LAT_STR}, {LON_STR}**
- Frame: **7 km × 9 km**, portrait, north-up
- Reference-height view preserving the whole lake and nearby Skrzynki, Czersk Świecki, Lipinki and surrounding terrain.
...
"""
    (EXP / "EXPERIMENT_008_REPORT.md").write_text(report, encoding="utf-8")


def build_combined_zip() -> Path:
    out = EXP / f"TEST008_JEZIORO_STELCHNO_1990_2026_FULL_VIEW_{FRAME_LABEL}_{LAT_STR}_{LON_STR}.zip"
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        for extra in ("EXPERIMENT_008_REPORT.md","experiment.json","EVIDENCE_POLICY.json"):
            p=EXP/extra
