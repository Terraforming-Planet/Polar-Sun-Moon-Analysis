...
PC_TOKEN=PC_STAC.replace('/stac/v1','')+'/api/sas/v1/token/{collection}'
...
# Pond seed derived from repeated visual alignment in the 2 km crops; approximately 150 m west / 170 m north of crop center.
# This is explicitly retained as a geometry-under-review seed, not a claimed cadastral coordinate.
POND_X,POND_Y=CX-150.0,CY+170.0
...
def l2_item(record):
    ...
    data=request_json('POST',PC_SEARCH,json={'collections':[collection],'bbox':[LON-0.04,LAT-0.03,LON+0.04,LAT+0.03],'datetime':f'{dt}T00:00:00Z/{dt}T23:59:59Z','limit':50})
...
def asset_key(item,exact,common):
    ...
    for key,a in assets.items():
        for b in (a.get('eo:bands') or []):
            if str(b.get('common_name','')).lower() in [x.lower() for x in common]: return key
    return None
...
def rois(shape):
    ...
    pond=(np.abs(xs-POND_X)<=430)&(np.abs(ys-POND_Y)<=430)
...
def object_measure(ndwi,mndwi,clear,roi,seed,maxdist):
    ...
    low=min([a for a in areas if a>=0]+[boundary_low]); high=max(areas+[boundary_high])
...
def overlay(mndwi,pond,lake,clear,season,year,date_text):
    ...
    im=Image.fromarray(rgb).resize((800,800),Image.Resampling.NEAREST); d=ImageDraw.Draw(im); d.rectangle((0,0,799,45),fill='white'); d.text((8,12),f'{year} {date_text} {season} | blue Lake Kuchnia | cyan pond candidate',fill='black')
...
def analyze_record(season,record):
    ...
    pond=object_measure(ndwi,mndwi,clear,pond_roi,rc_for(POND_X,POND_Y,ndwi.shape),330)
    lake=object_measure(ndwi,mndwi,clear,lake_roi,rc_for(LAKE_X,LAKE_Y,ndwi.shape),450)
    q=float(np.mean(clear&(pond_roi|lake_roi))/max(np.mean(pond_roi|lake_roi),1e-9))
    ...
    return {'season':season,'year':int(record['year']),'date':record['date'],'platform':record.get('platform'),'display_native_resolution_m':record.get('native_resolution_m'),'measurement_grid_m':30,'selected_month':record.get('selected_month'),'fallback_month':record.get('is_fallback_month'),'clear_fraction_measurement_aoi':round(q,6),'confidence':conf,'pond_geometry_status':'provisional visual seed; manual polygon verification required before final claim','forest_pond':{k:v for k,v in pond.items() if k!='mask'},'lake_kuchnia':{k:v for k,v in lake.items() if k!='mask'},'overlay':ov,'measurement_item_id':item.get('id'),'measurement_collection':item.get('collection')}
...
def endpoint(rows,season,obj):
    ...
    return {'season':season,'object':obj,'area_1990_m2':a,'area_2026_m2':b,'loss_1990_to_2026_m2':loss,'loss_ha':loss/10000.0,'loss_percent':pct,'1990_uncertainty_m2':[by[1990][obj]['low_m2'],by[1990][obj]['high_m2']],'2026_uncertainty_m2':[by[2026][obj]['low_m2'],by[2026][obj]['high_m2']],'status':'preliminary_common30m_spectral_measurement'}
...
def main():
    ...
    out={'experiment_id':'001','method':'Original L2 spectral bands; MNDWI+NDWI; common 30 m grid for cross-era comparability; threshold sensitivity plus boundary uncertainty','aoi_center':{'lat':LAT,'lon':LON},'pond_seed':{'utm_x':POND_X,'utm_y':POND_Y,'status':'provisional manual visual seed; final geometry gate not yet passed'},'records':allrows,'endpoint_comparisons':endpoints,'failures':failures,'working_estimate_before_measurement':{'loss_m2':25000,'loss_ha':2.5,'loss_percent':'near 100%','status':'provisional'}}
