...
BRANCH = "experiment-005-jezioro-tarpno-53-497670-18-822712"
EXP = Path("experiments/experiment_005_jezioro_tarpno")
SEASONS = EXP / "seasonal_evidence"
TARGET = {"name": "Jezioro Tarpno", "lat": LAT, "lon": LON}
FRAME_WIDTH_M = 4000.0
FRAME_HEIGHT_M = 5000.0
FRAME_LABEL = "4x5km"
...
def normalize_season_output(name: str, result: dict) -> dict:
    ...
    new = root / f"TEST005_{name.upper()}_1990_2026_37_YEARS_{FRAME_LABEL}_{LAT:.6f}_{LON:.6f}.zip"
    manifest_path = root / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["experiment"] = "Experiment 005 - Jezioro Tarpno water and shoreline monitoring"
    ...
    manifest["aoi"] = {
        ...
        "purpose": "whole Jezioro Tarpno plus surrounding Grudziadz landscape visible in every annual frame",
    }
    manifest["dataset_role"] = "independent TerraWater lake comparison case"
    manifest["test_number"] = 5
    manifest["object_name"] = "Jezioro Tarpno"
...
def build_combined_zip() -> Path:
    out = EXP / f"TEST005_JEZIORO_TARPNO_1990_2026_FULL_LAKE_{FRAME_LABEL}_{LAT:.6f}_{LON:.6f}.zip"
    if out.exists():
        out.unlink()
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        for extra in ("EXPERIMENT_005_REPORT.md", "experiment.json", "EVIDENCE_POLICY.json"):
            p = EXP / extra
            if p.exists():
                zf.write(p, p.name)
    return out
...
def write_metadata(spring: dict, autumn: dict) -> None:
    ...
    policy = {
        "experiment_id": "005",
        "object": "Jezioro Tarpno",
        "center": {"lat": LAT, "lon": LON},
        "target": TARGET,
        "frame": {"width_m": 4000, "height_m": 5000, "orientation": "portrait_north_up"},
        ...
        "rules": {
            ...
            "whole_lake_must_be_inside_frame": True,
    ...
    report = f"""# Experiment 005 — Jezioro Tarpno satellite evidence 1990–2026

## AOI
- Object: **Jezioro Tarpno**
- Center: **{LAT:.6f}, {LON:.6f}**
- Frame: **4 km × 5 km**, portrait, north-up
- Purpose: keep the whole lake and surrounding Grudziadz landscape visible in every annual image.
...
"""
    (EXP / "EXPERIMENT_005_REPORT.md").write_text(report, encoding="utf-8")
