...
NOAA_CDN = "https://cdn.star.nesdis.noaa.gov/GOES19/ABI/FD/GEOCOLOR"
PRODUCT = "GOES19-ABI-FD-GEOCOLOR-1808x1808.jpg"
OUTPUT_DIR = Path("web/public/eclipse/2026-08-12/goes19")
...
def frame_url(value: datetime) -> str:
    stamp = noaa_timestamp(value)
    return f"{NOAA_CDN}/{stamp}_{PRODUCT}"
...
def load_manifest() -> dict[str, object]:
    if not MANIFEST_PATH.exists():
        return {
            ...
            "source": "NOAA NESDIS STAR GOES-19 ABI Full Disk GeoColor",
            "source_page": "https://www.star.nesdis.noaa.gov/GOES/fulldisk.php?sat=G19",
            "product": "GeoColor (true color daytime, multispectral IR at night)",
            "cadence_minutes": 10,
            "image_size_px": [1808, 1808],
            "satellite_photography": True,
            "synthetic": False,
            "frames": [],
        }
...
def store_frame(now: datetime) -> tuple[Path, bool]:
    ...
    filename = f"{observed:%Y%m%dT%H%M%SZ}_GOES19_GEOCOLOR_1808.jpg"
