...
class TorchBatchTrainer:
    ...
    def __call__(self, batch: list[dict[str, Any]], state: StreamState) -> dict[str, Any]:
        ...
        arrays = [
            np.concatenate(cast(tuple[np.ndarray, np.ndarray], row["arrays"])) for row in batch
        ]
        tensor = self.torch.from_numpy(np.stack(arrays)).to(
            device=self.device, dtype=self.torch.float32, non_blocking=True
        )
        tensor = self.torch.nan_to_num(tensor, nan=0.0, posinf=0.0, neginf=0.0)
        target = self.torch.zeros(len(batch), dtype=self.torch.long, device=self.device)
        self.optimizer.zero_grad(set_to_none=True)
        loss = self.torch.nn.functional.cross_entropy(self.model(tensor), target)
        ...
        return {
            ...
            "objective": "representation pseudo-task; training loss is not environmental truth",
            "steps": self.steps,
            "loss_min": min(values),
